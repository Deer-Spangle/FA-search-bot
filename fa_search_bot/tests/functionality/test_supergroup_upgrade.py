...
def test_supergroup_upgrade_old_chat(context):
    old_chat_id = 12345
    new_chat_id = 54321
    update = MockTelegramEvent.with_message(chat_id=new_chat_id, migrate_from_chat_id=old_chat_id)
    api = MockExportAPI()
...
def test_supergroup_upgrade_new_chat(context):
    old_chat_id = 12345
    new_chat_id = 54321
    update = MockTelegramEvent.with_message(chat_id=old_chat_id, migrate_to_chat_id=new_chat_id)
    api = MockExportAPI()
    watcher = SubscriptionWatcher(api, context.bot)
    func = SupergroupUpgradeFunctionality(watcher)
    migrate_chat = MockMethod("Migrate subscriptions")
