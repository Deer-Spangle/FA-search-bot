...
def test_call__route_add_blocklisted_tag(context):
    update = MockTelegramEvent.with_channel_post(chat_id=-10014358, text="/add_blocklisted_tag test")
    api = MockExportAPI()
...
def test_call__route_remove_blocklisted_tag(context):
    update = MockTelegramEvent.with_channel_post(chat_id=-10014358, text="/remove_blocklisted_tag example")
    api = MockExportAPI()
    watcher = SubscriptionWatcher(api, context.bot)
    func = BlocklistFunctionality(watcher)
...
def test_call__route_list_blocklisted_tags(context):
    update = MockTelegramEvent.with_channel_post(chat_id=-10014358, text="/list_blocklisted_tags")
    api = MockExportAPI()
    watcher = SubscriptionWatcher(api, context.bot)
    func = BlocklistFunctionality(watcher)
