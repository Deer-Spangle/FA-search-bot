...
def test_call__route_add_blacklisted_tag(context):
    ...
    assert context.bot.send_message.call_args[1]['text'] == "Added to blacklist: test"


def test_call__route_remove_blacklisted_tag(context):
    ...
    context.bot.send_message.assert_called()
    assert context.bot.send_message.call_args[1]['chat_id'] == update.message.chat_id
    assert context.bot.send_message.call_args[1]['text'] == "Removed from blacklist: example"


def test_call__route_list_blacklisted_tags(context):
    ...
    context.bot.send_message.assert_called()
    assert context.bot.send_message.call_args[1]['chat_id'] == update.message.chat_id
    assert context.bot.send_message.call_args[1]['text'] == "Listing blacklisted tags"


def test_add_to_blacklist__no_add_blank(context):
    api = MockExportAPI()
    watcher = SubscriptionWatcher(api, context.bot)
    func = BlacklistFunctionality(watcher)
...
def test_add_to_blacklist__creates_blacklist_for_channel(context):
    api = MockExportAPI()
    watcher = SubscriptionWatcher(api, context.bot)
    func = BlacklistFunctionality(watcher)
    list_tags = MockMethod("Listing blacklisted tags")
...
def test_add_to_blacklist__add_tag_to_blacklist(context):
    api = MockExportAPI()
    watcher = SubscriptionWatcher(api, context.bot)
...
def test_remove_from_blacklist__tag_not_in_blacklist(context):
    api = MockExportAPI()
    watcher = SubscriptionWatcher(api, context.bot)
    watcher.blacklists[18749] = {"example"}
...
def test_remove_from_blacklist__removes_tag_from_blacklist(context):
    api = MockExportAPI()
    watcher = SubscriptionWatcher(api, context.bot)
...
def test_list_blacklisted_tags(context):
    api = MockExportAPI()
    watcher = SubscriptionWatcher(api, context.bot)
